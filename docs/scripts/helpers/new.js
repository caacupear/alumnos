...
async function prompt(config) {
    ...
    if ( !config.template ) {
        const response = await prompts({
            ...
            message: templateInexistente ? 'El template no es valido, seleccione uno por favor' :'Seleccione un template',
...
function help() {
    console.log('Este comando es un boilerplate de documentos basados en los templates de la carpeta scripts/templates/create.');
    console.log('El objetivo es que antes de crear un componente de documentacion se pueda armar es esqueleto de forma estandar.');
    console.log('Por ejemplo si se quiere documentar los servicios de un proceso se puede correr:');
...
function execute({template, filename, context}) {
